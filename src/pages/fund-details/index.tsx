...
const WebSignalDetails = () => {
  ...
  useEffect(() => {
    Taro.setNavigationBarTitle({
      title: router.params.title || "基金理财助手",
    });
  }, []);
  ...
  useEffect(() => {
    if (+userType == 2) {
      ...
      (async () => {
        ...
        let url =
          h5BaseURL +
          `fund-details/index?token=${getLocalToken()}&fund_code=${fund_code}&chn=mini_program&did=${Taro.getStorageSync(
            "did"
          )}&isFree=${true}`;
